...
export default function SettingsPage() {
 ...
 <div className="w-32 h-36 bg-[#e8e4d9] border-2 border-stone-950 p-1 mx-auto relative shadow-sm">
 <div className="w-full h-full bg-[#dcd7c9] flex flex-col items-center justify-center relative overflow-hidden">
 ...
 <div key={session.id} className="bg-[#fcfaf2] border-2 border-stone-950 p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:bg-[#f0ead6] transition-all">
 <div>
 <div className="text-stone-950 text-xs font-bold tracking-widest flex items-center gap-2 font-mono">
 NODE_TETHER_{idx + 1}
 {session.isCurrent && (
 <span className="text-[8px] bg-stone-950 text-[#fcfaf2] px-1.5 py-0.5 uppercase font-mono">CURRENT</span>
 )}
 </div>
 ...
 {/* Form details */}
 <div className="bg-[#fcfaf2] border-2 border-stone-950 p-4">
 ...
 {/* whitelist entries */}
 <div className="bg-[#f5f2e9] border border-stone-300 p-4 flex flex-col">
 ...
 {whitelist.map((w) => (
 <div key={w.id} className="font-mono text-[11px] text-stone-850 flex justify-between items-center group/wl hover:bg-[#fcfaf2] px-2 py-1.5 border border-stone-350 bg-[#fcfaf2]/40">
 <span className="truncate flex-1 min-w-0 mr-2 font-bold">{w.email}</span>
 <button
 onClick={() => handleDeleteWhitelist(w.id)}
 className="text-[9px] text-red-800 hover:text-red-950 font-bold opacity-0 group-hover/wl:opacity-100 transition-opacity border-2 border-red-800 px-1.5 py-0.5 bg-[#fcfaf2] cursor-pointer"
 >
 REVOKE ID
 </button>
 </div>
