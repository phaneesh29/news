...
export const requestOtp = async (c: Context) => {
  ...
  if (!await consumeRateLimit(requestKey, 5, OTP_TTL_MS, 'rl:admin:otp-request')) {
    throw new HTTPException(429, { message: 'Too many OTP requests. Please try again later.' })
  }
  ...
  if (whitelisted.length === 0) {
    throw new HTTPException(403, { message: 'Email is not authorized' })
  }
  ...
  if (!user) {
    throw new HTTPException(500, { message: 'User could not be created' })
  }

  if (user.status !== 'active') {
    throw new HTTPException(403, { message: 'User account is not active' })
  }
...
export const verifyOtp = async (c: Context) => {
  ...
  if (!await consumeRateLimit(verifyKey, 10, OTP_TTL_MS, 'rl:admin:otp-verify')) {
    throw new HTTPException(429, { message: 'Too many verification attempts. Please try again later.' })
  }
  ...
  if (!latestOtp) {
    throw new HTTPException(401, { message: 'No OTP found. Please request a new one.' })
  }

  if (latestOtp.expiresAt < new Date()) {
    throw new HTTPException(401, { message: 'OTP has expired' })
  }

  if (latestOtp.attempts >= latestOtp.maxAttempts) {
    throw new HTTPException(401, { message: 'Maximum attempts reached. Please request a new OTP.' })
  }
